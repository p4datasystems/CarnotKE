...
public class SPARQLDoer {
    ...
    public static void createQuadStore(PyRelConnection connection) {
        String s = "DECLARE\ncnt NUMBER;\n";
        s += "BEGIN\nSELECT count(*) INTO cnt FROM MDSYS.SEM_MODEL$ WHERE MODEL_NAME = '" + connection.getModel().toUpperCase() + "';\n";
        s += "IF ( cnt = 0 )\nTHEN\n";
        s +=
    ...
    public static void deleteQuadsWithSubjectProp(PyRelConnection connection, 
        ...
        try {
			...
			while (rs.next()) {
				...
				String quadName = connection.getModel() + ":<" + connection.getGraph() + ">";
				String deleteSql =
					"DELETE FROM "+connection.getTable()+" WHERE TRIPLE = SDO_RDF_TRIPLE_S('" + quadName + "', '" + connection.getNamespace() +
