...
/**
 * Created by Alvin Deng on 3/31/16.
 */

public class CarnotHelper extends SelectDeParser implements SelectVisitor, FromItemVisitor, ExpressionVisitor, ItemsListVisitor, SelectItemVisitor, OrderByVisitor {

    /* SIMHelper attributes */
    public PyRelConnection connection = null;
    final String schemaString = "SCHEMA";
    String NoSQLNameSpacePrefix = "";

    /* SQLVisitor attributes */
    ...
    /* Boolean to indicate weather or not we are currently at a subquery */
    ...
    List<String> subColumns = null;
    private String subqStr = "";
    ...
    ArrayList<String> subAllCols = new ArrayList<String>();
    private static String[] aggregates = {"avg", "count", "max", "min", "sum"}; // Input as lower case.
    private static String[] inequalities = {" = ", " > ", " < ", " >= ", " <= ", " != ", " in ", " IN "};
    ...
    /**
     * Build a CarnotHelper to process SIM commands and execute them against a connection to
    ...
    /* Shared by SIMHelper and SQLVistor */
    public CarnotHelper(PyObject conn)
    {
        /* SIMHelper logic */
    ...
    public String visitSelect_buildSPARQL(PlainSelect plainSelect) {
        ...
        //if (subFromAfter) subColumnsAs = columnsAs;
        ...
        if (plainSelect.getJoins() != null) {
            ...
            for (Iterator joinsIt = plainSelect.getJoins().iterator(); joinsIt.hasNext(); ) {
                ...
                if (join.getOnExpression() != null) {
                    ...
                    if ((joinStr.indexOf(x) >= 0) || (joinStr.indexOf(x.toUpperCase()) >= 0)) {

                        getSubStructures(tables, tablesAliases, tables2alias, columnsAs, aggrColumnsAs, tableSymbols);

                        String subsq = joinStr.substring(joinStr.split("ON")[0].indexOf("(") + 1, joinStr.split("ON")[0]
                                .indexOf(")"));
                        if (col1.equals("")) col1 = col2;
                        String cols = subsq.split("FROM ")[0];
                        cols = cols.split("SELECT ")[1].split(" ")[0];
                        if (cols.indexOf(col1.split("\\.")[1]) < 0)
                            System.out.println("This is not a valid Join subquery");
                        else {
                ...
                for (String ineq : inequalities) {
                    if (c.contains(ineq)) {
                        ...
                        if (left.equals(""))
                            ...
                            fColumns += "?" + tableSymbols.get(tablesAliases.get(left.split("\\.")[0])) + " " + NoSQLNameSpacePrefix + ":" + left
                                    .split("\\.")[1] + " ?f" + n + " .\n";
                            filter += "?f" + n;
                            n++;
                        }
                        ...
                        if (right.equals(""))
                            ...
                            // So fColumns will be set = ?s1 c:eventType ?f1 .
                            filter += "?f" + n;
                            n++;
                        }
                        String x = "select";
                        if (!subFromAfter) {
                            if ((c.indexOf(x) >= 0) || (c.indexOf(x.toUpperCase()) >= 0)) {
                                ...
                                try {
                                    ...
                                    PlainSelect plainSelect2 = (PlainSelect) ((Select) parserManager.parse(new StringReader(subsq)))
                                            .getSelectBody();
                                    visit(plainSelect2);
        ...
        if (plainSelect.getOrderByElements() != null) {
            ...
            for (int i = 0; i < orderbyElems.length; i++) {
                ...
                if ((i + 1 < orderbyElems.length) && (orderbyElems[i + 1].equalsIgnoreCase("ASC") || orderbyElems[i + 1]
        ...
        for (String s : joinColumns) {
            if (!connection.getConnectionDB().equals("OracleNoSQL")) {
                ...
                SPARQL += "\t?" + tableSymbols.get(tablesAliases.get(s.split(" = ")[0].split("\\.")[0])) + " " + NoSQLNameSpacePrefix + ":" + s
                        .split(" = ")[0].split("\\.")[1] + " ?j" + n + " .\n";
                // This will get the symbol from tableSymbols for the d from d.n and will get n from d.n
                SPARQL += "\t?" + tableSymbols.get(tablesAliases.get(s.split(" = ")[1].split("\\.")[0])) + " " + NoSQLNameSpacePrefix + ":" + s
                        .split(" = ")[1].split("\\.")[1] + " ?j" + n + " .\n";
            } else {

                // This will get the symbol from tableSymbols for the e from e.n and will get n from e.n
                SPARQL += "\n\t\tGRAPH ?gj" + n + " { ?" + tableSymbols.get(tablesAliases.get(s.split(" = ")[0].split("\\.")[0])) + " " + NoSQLNameSpacePrefix + ":" + s
                        .split(" = ")[0].split("\\.")[1] + " ?j" + n + " . } ";
                // This will get the symbol from tableSymbols for the d from d.n and will get n from d.n
                SPARQL += "\n\t\tGRAPH ?gj" + (n + 1) + " { ?" + tableSymbols.get(tablesAliases.get(s.split(" = ")[1].split("\\.")[0])) + " " + NoSQLNameSpacePrefix + ":" + s
                        .split(" = ")[1].split("\\.")[1] + " ?j" + n + " .  } ";
            }
            n++;
        }
        ...
        if (connection.getConnectionDB().equals("OracleNoSQL")) SPARQL += "";
            ...
            SPARQL += "\n" + "' ,\nSEM_MODELS('" + connection.getModel() + "'), null,\nSEM_ALIASES( SEM_ALIAS('', '" + connection
                    .getNamespace() + "')), null) )";
     ...
     */
    /* SQLVisitor method */
    public List<String> getSubjects(PyRelConnection connection, String graph, String predicate, String object) throws SQLException {
     ...
     */
    /* SQLVisitor method */
    public void insertQuad(PyRelConnection connection, String graph, String subject, String predicate, String object, Boolean eva)
            throws SQLException {
